...
/* === Language / i18n — URL-based bilingual navigation === */
(function () {
  // Determine current language from the <html lang> attribute (set server-side)
  var currentLang = document.documentElement.lang || 'en';
  var isOnFrench = currentLang === 'fr';

  // Language toggle navigates to the correct URL instead of swapping content client-side.
  // EN lives at / and FR lives at /fr/ — each page is pre-rendered with the right content.
  document.querySelectorAll('.lang-btn').forEach(function (btn) {
    btn.addEventListener('click', function () {
      var targetLang = this.getAttribute('data-lang');
      if (targetLang === currentLang) return;

      try { localStorage.setItem('ldc_lang', targetLang); } catch (e) {}

      if (targetLang === 'fr') {
        window.location.href = '/fr/' + window.location.hash;
      } else {
